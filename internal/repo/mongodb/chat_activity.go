...
type ChatActivityRepository interface {
	...
	GetByRoomID(ctx context.Context, roomID string, limit int) ([]*models.ChatActivity, error)
}
...
func (r *chatActivityRepo) GetByRoomID(ctx context.Context, roomID string, limit int) ([]*models.ChatActivity, error) {
	...
	cursor, err := r.collection.Find(ctx, filter, opts)
	if err != nil {
		return nil, fmt.Errorf("failed to get activities by room: %w", err)
	}
	defer cursor.Close(ctx)
